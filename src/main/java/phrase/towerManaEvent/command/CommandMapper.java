...
public class CommandMapper {
    ...
    public CommandResult mapCommand(Player player, String label, String[] args) {
        ...
        if(commandDescription == null) return new CommandResult(messages.unknownCommand(), CommandResult.ResultStatus.UNKNOWN_COMMAND);

        if(!player.hasPermission(commandDescription.getPermission())) return new CommandResult(messages.noPermission(), CommandResult.ResultStatus.NO_PERMISSION);
        ...
        if(!success) return new CommandResult(messages.incorrectArguments(), CommandResult.ResultStatus.INCORRECT_ARGUMENTS);
